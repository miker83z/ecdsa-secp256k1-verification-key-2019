...
class Secp256k1KeyPair {
  ...
  static async generate(options = {}) {
    ...
    const publicKeyBase58 = base58.encode(
      new Uint8Array(pubPoint.encodeCompressed())
    );
    const privateKeyBase58 = base58.encode(
      new Uint8Array(key.getPrivate().toArray())
    );
  ...
  /**
   * Generates a KeyPair with an optional deterministic seed
   * from Uint8Arrays of public key and private key.
   * @param {Uint8Array} publicKeyUint8Compressed - The public key to use.
   * @param {Uint8Array} privateKeyUint8 - The private key to use.
   * @param {KeyPairOptions} [options={}] - The options to use.
   *
   * @returns {Promise<Secp256k1KeyPair>} Generates a key pair.
   */
  static async generateFromUint8Keypair(
    publicKeyUint8Compressed,
    privateKeyUint8,
    options = {}
  ) {
    // encode public X and Y in compressed form
    const publicKeyBase58 = base58.encode(publicKeyUint8Compressed);
    const privateKeyBase58 = base58.encode(privateKeyUint8);
    return new Secp256k1KeyPair({
      privateKeyBase58,
      publicKeyBase58,
      ...options,
